...
def pad_center(x, size, axis=-1):
    """
    Wrapper around librosa.util.pad_center for newer librosa versions.
    In recent librosa, the signature is pad_center(data, *, size=None, axis=-1),
    so `size` must be passed as a keyword argument.
    """
    return librosa.util.pad_center(x, size=size, axis=axis)
